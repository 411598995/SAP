...
namespace ACHR.Screen
{
    class frm_ProdList : HRMSBaseForm
    {
        ...
        public override void etAfterClick(ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
        {
            ...
            if (pVal.ItemUID != itemClicked) oApplication.MessageBox("Drag Drop");
        ...
        public override void etAfterValidate(ref SAPbouiCOM.ItemEvent pVal, ref bool BubbleEvent)
        {
            ...
            if (pVal.ItemUID == mtReport.Item.UniqueID)
            {
                if ( pVal.ColUID == "cFT")
                {
                    ...
                    if (selRowIndex >= 0)
                    {
                        string DocEntry = Convert.ToString(dtRpt.GetValue("DocNum", selRowIndex));
            ...
            if (confirmresult == 1)
            {
                ...
                for (int i = 1; i <= mtReport.RowCount; i++)
                {
                    if (mtReport.IsRowSelected(i))
                    {
                        ...
                        string DocEntry = Convert.ToString(dtRpt.GetValue("DocNum", selRowInd));
                        prodOrder.GetByKey(Convert.ToInt32(DocEntry));
                        if (selButton == "01")
                        {
        ...
        private void assignPL()
        {
            ...
            if (confirmresult == 1)
            {
                ...
                string maxSeqNum = Program.objHrmsUI.getScallerValue("Select max(U_B1_Seq) from owor where U_PMX_PLCD='" + selPL + "'");
        ...
        private void fillReport()
        {
          ...
          //  strCritaria = " Where 'Y' = 'Y'  ";
            ...
            string strSelect = "Select owor.docnum, case  owor.type when 'S' then 'Standard' when 'P' then 'Special' when 'D' then 'Disassembly' else 'UD' end as Type , case owor.status when 'P' then 'Planned' when 'R' then 'Released' else 'Closed' end as Status,owor.itemcode,oitm.itemname " ;
