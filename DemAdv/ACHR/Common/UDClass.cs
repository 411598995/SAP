...
namespace ACHR
{
    ...
    public class UDClass
    {
        ...
        public void createConfigurationTables()
        {
            ...
            AddTable("DEM_MODULES", "Demmex Modules", BoUTBTableType.bott_NoObject);
            AddColumns("@DEM_MODULES", "Enabled", "Enabled", SAPbobsCOM.BoFieldTypes.db_Alpha, 1, SAPbobsCOM.BoFldSubTypes.st_None);
            AddColumns("@DEM_MODULES", "LicenseKey", "License Key", SAPbobsCOM.BoFieldTypes.db_Alpha, 100, SAPbobsCOM.BoFldSubTypes.st_None);

            //AddTable("B1_DEM_SETTING", "DEM Settings", BoUTBTableType.bott_NoObjectAutoIncrement);
            //AddColumns("@B1_DEM_SETTING", "Value", "Value", SAPbobsCOM.BoFieldTypes.db_Alpha, 254, SAPbobsCOM.BoFldSubTypes.st_None, "");


            addModulesRows();

            System.Data.DataTable dtModules = getDataTable("Select * from [@DEM_MODULES] ", "Getting Modules");
            DemModules.Clear();
        ...
        {
            int cntExist = Convert.ToInt32(getScallerValue("Select count(*) from [@DEM_MODULES] where Code = 'SubGrp'"));
            if (cntExist == 0)
            {
                string strInsert = "Insert into [@DEM_MODULES](Code,Name,U_Enabled,U_LicenseKey) values ('SubGrp','Sub Group Module','N','NA')";
                ExecQuery(strInsert, "ModuleRow");


            }
            cntExist = Convert.ToInt32(getScallerValue("Select count(*) from [@DEM_MODULES] where Code = 'ProdSeq'"));
            if (cntExist == 0)
            {
                string strInsert = "Insert into [@DEM_MODULES](Code,Name,U_Enabled,U_LicenseKey) values ('ProdSeq','Production Sequencer Module','N','NA')";
                ExecQuery(strInsert, "ModuleRow");


            }
            cntExist = Convert.ToInt32(getScallerValue("Select count(*) from [@DEM_MODULES] where Code = 'LaborReg'"));
            if (cntExist == 0)
            {
                string strInsert = "Insert into [@DEM_MODULES](Code,Name,U_Enabled,U_LicenseKey) values ('LaborReg','Labor Registration Module','N','NA')";
                ExecQuery(strInsert, "ModuleRow");


            }
            cntExist = Convert.ToInt32(getScallerValue("Select count(*) from [@DEM_MODULES] where Code = 'CRM'"));
            if (cntExist == 0)
            {
                string strInsert = "Insert into [@DEM_MODULES](Code,Name,U_Enabled,U_LicenseKey) values ('CRM','Sales Tool','N','NA')";
                ExecQuery(strInsert, "ModuleRow");


            }
            cntExist = Convert.ToInt32(getScallerValue("Select count(*) from [@DEM_MODULES] where Code = 'ProdCost'"));
            if (cntExist == 0)
            {
                string strInsert = "Insert into [@DEM_MODULES](Code,Name,U_Enabled,U_LicenseKey) values ('ProdCost','Production Cost Module','N','NA')";
                ExecQuery(strInsert, "ModuleRow");


            }

            cntExist = Convert.ToInt32(getScallerValue("Select count(*) from [@DEM_MODULES] where Code = 'MRPD'"));
            if (cntExist == 0)
            {
                string strInsert = "Insert into [@DEM_MODULES](Code,Name,U_Enabled,U_LicenseKey) values ('MRPD','Planning for Disassembly','N','NA')";
                ExecQuery(strInsert, "ModuleRow");


            }

            cntExist = Convert.ToInt32(getScallerValue("Select count(*) from [@DEM_MODULES] where Code = 'SaleOrder'"));
            if (cntExist == 0)
            {
                string strInsert = "Insert into [@DEM_MODULES](Code,Name,U_Enabled,U_LicenseKey) values ('SaleOrder','Sale Order Messaging','N','NA')";
                ExecQuery(strInsert, "ModuleRow");
        ...
        {
            settings.Clear();
            string strSetting = "Select * from  \"@B1_DEM_SETTING\" ";
        ...
        public void SaveSetting(string name, string value)
        {
            ...
            string strInsert = "";
            strInsert = "IF  EXISTS(SELECT * FROM \"@B1_DEM_SETTING\"  WHERE \"Name\" ='" + name + "') UPDATE  \"@B1_DEM_SETTING\"   SET \"U_Value\" =  '" + value.Replace("'", "''") + "'  WHERE  \"Name\" =  '" + name + "';";
            ExecQuerySilent(strInsert, "Add Default");
            strInsert = "IF NOT EXISTS(SELECT * FROM \"@B1_DEM_SETTING\"  WHERE \"Name\" ='" + name + "') INSERT INTO \"@B1_DEM_SETTING\" ( \"Name\", \"U_Value\") VALUES ( '" + name.Replace("'", "''") + "', '" + value.Replace("'", "''") + "');";
            ExecQuerySilent(strInsert, "Add Default");
        ...
        public void loadMenu(string lang)
        {
            ...
            if (mnus.Exists("DEM_ADV"))
            {
                mnus.RemoveEx("DEM_ADV");
            }
            mnus.Item("43523").SubMenus.Add("DEM_ADV", "Demmex Advance", BoMenuType.mt_POPUP, 12);
            mnus.Item("DEM_ADV").SubMenus.Add("mnu_DEMSetup", "Module Setup", BoMenuType.mt_STRING, 2);
