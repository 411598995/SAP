...
namespace ACHR.Screen
{
    ...
    class frm_SS : HRMSBaseForm
    {
        ...
        private void fillCustOrderList(string cardCode)
        {
            ...
            strSelect = @"select top " + drSetting["U_NLastOrdr"].ToString() + @" ordr.docnum [Order],ordr.docdate [Date],rdr1.itemcode ,rdr1.Dscription as  [Product] ,rdr1.quantity [Quantity] , 0 [WT] ,rdr1.Price [Price] 
        ...
        private void fillOffers(string cardCode)
        {
            ...
            strSelect = @"SELECT        dbo.SPP1.ItemCode, dbo.OITM.ItemName, ISNULL(dbo.SPP2.Amount, 1) AS Quantity, ISNULL(dbo.SPP2.Price, dbo.SPP1.Price) AS Price, ISNULL(dbo.SPP2.Discount, dbo.SPP1.Discount) AS Discount
                            FROM            dbo.SPP1 INNER JOIN
                                                     dbo.OITM ON dbo.SPP1.ItemCode = dbo.OITM.ItemCode INNER JOIN
                                                     dbo.OSPP ON dbo.SPP1.ItemCode = dbo.OSPP.ItemCode AND dbo.SPP1.CardCode = dbo.OSPP.CardCode LEFT OUTER JOIN
                                                     dbo.SPP2 ON dbo.SPP1.ItemCode = dbo.SPP2.ItemCode AND dbo.SPP1.LINENUM = dbo.SPP2.SPP1LNum
                            WHERE        (dbo.OSPP.ListNum = " + listNum + @")
                          ";
        ...
        private void fillStockVal(string itemCode)
        {
            ...
            strSelect = "		select oitm.onhand,oitm.IsCommited,oitm.OnOrder , oitm.AvgPrice,SalUnitMsr from oitm where oitm.ItemCode='" + itemCode + "'   ";
            ...
            foreach (System.Data.DataRow dr in dtProducts.Rows)
            {
                ...
                dtStock.SetValue("onhand", j, dr["onhand"].ToString());
                dtStock.SetValue("comited", j, dr["IsCommited"].ToString());
                dtStock.SetValue("ordered", j, dr["OnOrder"].ToString());

                dtStock.SetValue("avgprice", j, dr["AvgPrice"].ToString());
                dtStock.SetValue("UOM", j, dr["SalUnitMsr"].ToString());
